...
rm -rf generated/benchmark results/raw results/normalized results/metrics
rm -f evidence/evaluation/command-log.txt evidence/evaluation/SHA256SUMS
mkdir -p results/raw results/normalized results/metrics evidence/evaluation reports

run_logged() {
  local gitleaks_unredacted trufflehog_unredacted
  local gitleaks_status trufflehog_status detect_secrets_status
  ...
  /usr/bin/time -f '{"elapsed_seconds":%e,"max_rss_kb":%M,"cpu_percent":"%P","exit_status":%x}' \
    ...
    timeout 180 "$TRUFFLEHOG_BIN" filesystem generated/benchmark/cases \
      ...
      > "$trufflehog_unredacted" 2> results/raw/trufflehog-stderr.txt
  trufflehog_status=$?
...
# The finite pipeline closes tee before checksums are generated. No process may
# mutate command-log.txt between checksum creation and verification.
run_logged 2>&1 | tee -a evidence/evaluation/command-log.txt
